...
def is_bool(expr: str, si: int) -> str:
    asm: str = ''
    asm += cmp.compile_expr(pf.args_list(expr)[0])
...
def is_char(expr: str, si: int) -> str:
    asm: str = ''
    asm += cmp.compile_expr(pf.args_list(expr)[0])
...
def is_zero(expr: str, si: int)->str:
    asm: str = ''
    asm += cmp.compile_expr(pf.args_list(expr)[0])
    asm += emit_is_eax_equal_to('0')
    return asm
